...
# 目录
# [0 命令行解释器](0-命令行解释器)
  - ## [0.1 bash配置文件](#01-bash配置文件)
  - ## [0.2 shell脚本管理](#02-shell脚本管理)
  - ## [0.3 shell脚本语言](#03-shell脚本语言)
# [1 系统相关](#1-系统相关)
  - ## [1.1 系统开关](#11-系统开关)
  - ## [1.2 系统监控](#12-系统监控)
  - ## [1.3 内存管理](#13-内存管理)
  - ## [1.4 环保变量管理](#14-环境变量管理)
# [2 用户相关](#2-用户相关)
  - ## [2.1 用户信息查询](#21-用户信息查询)
  - ## [2.2 用户切换](#22-用户切换)
  - ## [2.3 用户修改](#23-用户修改)
  - ## [2.4 用户信息修改](#24-用户信息修改)
  - ## [2.5 用户组](#25-用户组)
# [3 文件相关](#3-文件相关)
  - ## [3.1 文件信息查询](#31-文件信息查询)
  - ## [3.2 文件修改](#32-文件修改)
  - ## [3.3 文件信息修改](#33-文件信息修改)
  - ## [3.4 打开文件](#34-打开文件)
# [4 软件相关](#4-软件相关)
  - ## [4.1 下载安装软件](#41-下载安装软件)
  - ## [4.2 更新软件](#42-更新软件)
  - ## [4.3 卸载软件](#43-卸载软件)
  - ## [4.4 软件进程](#44-软件进程)
# [5 硬件管理](#5-硬件管理)
...
# 0 命令行解释器  
## 0.1 bash配置文件
默认用户配置文件为"/.bash_profile"和"~/.bashrc",通常将环境变量和登录初始化放在前者,交互配置放在后者,并在前者中显式调用后者.
  - 文件"/.bash_profile":
...
根据shell脚本的用途与类型,将shell脚本分成不同的种类,存放在不同的目录
  ...
  3. 应用程序脚本:
    ...
    - 如,~/.cargo/bin
  3. 临时脚本: 临时或一次性脚本存放在/tmp目录,系统可能会自动清理该目录下的文件
  4. 项目脚本: 放在项目目录内部,和项目代码存放在一起
...
> 启动其他官方库:`$ sudo vim /etc/pacman.conf` 去掉要启用库的前面注释 
...
`top | grep helix`: 查询"helix"软件的进程信息

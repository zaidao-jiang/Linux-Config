...
  `$ eval $(ssh-agent -s)`  
  2. 添加私钥到ssh-agent  
  `$ ssh-add -c -t 1h ~/.ssh/钥匙名称`
  3. 清除加载到ssh-agent的密钥
  `$ ssh-add -D`

通过bash配置文件(.bashrc)配置命令别名
  `alias rsa='eval $(ssh-agent -s)'
  `alias asa='ssh-add -c -t 1h ~/.ssh/key'
  `alias dsa='ssh-add -D'
...
挂载命令示例：`sudo mount -t exfat -o uid=$(id -u),gid=$(id -g),umask=022 /dev/sda1 /mnt`
取消挂载命令示例：`sudo umount -l /mnt`

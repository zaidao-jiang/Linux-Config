Author:maodun
Modifydate:2025.7.7
Description:hardware device configuration
...
> "ssh-add -c": 这会要求 ssh-agent 在每次使用密钥进行认证时都向您弹出确认提示（通常通过图形界面或终端）  
...
通过以下步骤安装并启动声卡驱动:
  1. 安装声音驱动  
  `$ sudo pacman -S pipewire pipewire-audio pipewire-alsa pipewire-pulse piprwire-jack`
  2. 电脑重启启动驱动
  3. 检查驱动情况
  `$ pactl info`
...
  - 调节声卡音量  
  `$ pactl set-sink-volume {sink} 音量百分比/+百分比(增加百分比音量)`  
  - 控制声卡是否静音  
  `$ pactl set-sink-mute {sink} toggle`  

> 可通过"pactl list sinks"获取声卡名称
> 要使用默认声卡可输入{@DEFAULT\_SINK@}
...
  `$ sudo fdisk -l`  
  3. 挂载U盘到挂载点  
  `$ sudo mount -t ntfs-3g /dev/“U盘分区名称” /“挂载路径” --mkdir`
  4. 取消挂载  
  `$ sudo umount “/挂载路径” ` 

Author:maodun
Modifydate:2025.7.7
Description:archlinux minimal installation
...
# 1 安装前准备
## 1.1 获取安装包
从[官方下载地址](https:/archlinux.org/download/)获取iso镜像文件种子，使用[aria2](./5.常用软件.md)软件下载种子
...
通过以下步骤将文件烧录进U盘:
  ...
  `$ lsblk`
  2. 使用**dd**将iso文件写入U盘:
  `$ sudo dd if=路径/镜像.iso of=/dev/sdx bs=4m status=progress oflag=sync`
    - if=...: 指定iso文件路径与名称
    - of=...: 指定u盘的路径与名称
    - bs=4m: 自定义写入速度
...
通过以下步骤使用**timedatectl**命令更新系统时间:
  1. 设置时区
  `$ timedatectl set-timezone Asia/Shang hai`  
  2. 查看各时间  
  `$ timedatectl`  
...
  `$ pacstrap -K /mnt grub efibootmgr`  
  3. 安装其他需要的软件  
  `$ pacstrap -K /mnt networkmanager helix man-db man-pages`  
...
## 3.5 区域本地化设置
按照以下步骤设置:
  1. 使用[helix]()软件编辑"locale.gen"文件,取消en\_US.UTF-8 UTF-8前的注释符号
  `$ helix /etc/locale.gen`    
  2. 生成locale信息  
  `$ locale-gen`  
  3. 使用[helix]()软件编辑locale.conf文件, 添加LANG=en\_ US.UTF-8  
...
## 3.6 创建root用户
按照以下步骤创建:
  1. 使用[helix]()软件编辑hostname文件,添加主机名称  
...
  2. 设置相应用户密码  
  `$ passwd 用户名`  
  3. 赋予wheel组能够执行任何命令的权限,使用[helix]()软件编辑sudoers文件,去掉“%wheel ALL=（ALL:ALL）ALL”前的注释 
...
## 3.9 启用发现其他系统引导(选做)
使用[helix]()软件编辑"grub"文件，去掉“GRUB\_DISABLE\_OS\_PROBER=false”前的注释  

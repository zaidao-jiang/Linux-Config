...
  `$ pacstrap -K /mnt grub efibootmgr`  
  3. 安装其他需要的软件  
  `$ pacstrap -K /mnt networkmanager vim yazi man-db man-pages`  
...
## 3.5 区域本地化设置
按照以下步骤设置:
  1. 使用[vim]软件编辑"locale.gen"文件,取消en\_US.UTF-8 UTF-8前的注释符号
  `$ vim /etc/locale.gen`    
  2. 生成locale信息  
  `$ locale-gen`  
  3. 使用[vim]软件编辑locale.conf文件, 添加LANG=en\_US.UTF-8  
  `$ vim /etc/locale.conf`
...
## 3.6 创建root用户
按照以下步骤创建:
  1. 使用[vim]软件编辑hostname文件,添加主机名称  
...
  2. 设置相应用户密码  
  `$ passwd 用户名`  
  3. 赋予wheel组能够执行任何命令的权限,使用[vim]软件编辑sudoers文件,去掉“%wheel ALL=（ALL:ALL）ALL”前的注释 
